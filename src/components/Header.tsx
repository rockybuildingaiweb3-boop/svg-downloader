      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between py-2.5 gap-3 border-b border-slate-100">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Download CTA Dropdown (Tier 2 - Download UX) */}
            <div className="relative">
              <button
                id="btn-header-download-dropdown"
                onClick={() => setShowExportMenu(!showExportMenu)}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-xl text-white bg-slate-900 hover:bg-slate-800 transition-colors shadow-2xs cursor-pointer"
                title={t.header.downloadDropdownTitle}
              >
                <Download className="w-3.5 h-3.5 text-emerald-400" />
                <span>{t.card.downloadSvg}</span>
                <ChevronDown className="w-3 h-3 text-slate-400" />
              </button>
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-1.5 w-64 bg-white rounded-xl shadow-lg border border-slate-200 py-1.5 z-50 animate-in fade-in zoom-in-95 duration-100">
                    ...
                    >
                      ...
                      <div>
                        <span className="font-semibold text-slate-900 block">{t.card.downloadCanonical}</span>
                        <span className="text-slate-500 text-2xs block">
                          {t.header.downloadCanonicalTooltip}
                        </span>
                      </div>
                    </button>

                    {selectedCount > 0 && onDownloadSelectedZip && (
                      <button
                        onClick={() => {
                          onDownloadSelectedZip();
                          setShowExportMenu(false);
                      ...
                      >
                        ...
                        <div>
                          <span className="font-semibold text-slate-900 block">
                            {format(t.filters.selectedCount, { count: selectedCount })}
                          </span>
                          <span className="text-slate-500 text-2xs block">
                            {t.header.downloadSelectedTooltip}
                          </span>
                        </div>
                      </button>
                    )}
                    ...
                    {onDownloadRegistryJson && (
                      ...
                      >
                        ...
                        <div>
                          <span className="font-semibold text-slate-900 block">{t.header.downloadRegistryJson}</span>
                          <span className="text-slate-500 text-2xs block">
                            registry.json
                          </span>
                        </div>
